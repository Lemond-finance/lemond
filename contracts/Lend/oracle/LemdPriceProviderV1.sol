...
import "./LemdPriceOracleInterface.sol";
...
contract LemdPriceProviderV1 is Ownable {
    ...
    event ConfigUpdated(address lToken, address underlying, string underlyingSymbol, uint256 baseUnit, bool fixedUsd);
    event PriceOracleUpdated(address lToken, PriceOracle[] oracles);
    ...
    function getUnderlyingPrice(address _lToken) external view returns (uint){

        uint256 price = 0;
        TokenConfig storage tokenConfig = tokenConfigs[_lToken];
        if (tokenConfig.fixedUsd) {//if true,will return 1*e36/baseUnit
            price = 1;
            return price.mul(1e36).div(tokenConfig.baseUnit);
        }

        PriceOracle[] storage priceOracles = oracles[_lToken];
        for (uint256 i = 0; i < priceOracles.length; i++) {
            ...
            if (priceOracle.available == true) {// check the priceOracle is available
                price = _getUnderlyingPriceInternal(_lToken, tokenConfig, priceOracle);
                if (price > 0) {
                    return price;
                }
            }
    ...
    function _getUnderlyingPriceInternal(address _lToken, TokenConfig memory tokenConfig, PriceOracle memory priceOracle) internal view returns (uint){
    ...
    function _getCustomerPriceInternal(PriceOracle memory priceOracle, TokenConfig memory tokenConfig) internal view returns (uint) {
        ...
        LemdPriceOracleInterface customerPriceOracle = LemdPriceOracleInterface(source);
    ...
    function addTokenConfig(address lToken, address underlying, string memory underlyingSymbol, uint256 baseUnit, bool fixedUsd,
        ...
        // add TokenConfig
        TokenConfig storage tokenConfig = tokenConfigs[lToken];
        require(tokenConfig.lToken == address(0), "bad params");
        tokenConfig.lToken = lToken;
        ...
        // add priceOracles
        require(oracles[lToken].length < 1, "bad params");
        for (uint i = 0; i < sources.length; i++) {
            PriceOracle[] storage list = oracles[lToken];
        ...
        emit ConfigUpdated(lToken, underlying, underlyingSymbol, baseUnit, fixedUsd);
        emit PriceOracleUpdated(lToken, oracles[lToken]);

    }

    function addOrUpdateTokenConfigSource(address lToken, uint256 index, address source, PriceOracleType _sourceType, bool available) public onlyOwner {
    ...
    function updateTokenConfigBaseUnit(address lToken, uint256 baseUnit) public onlyOwner {
        TokenConfig storage tokenConfig = tokenConfigs[lToken];
        require(tokenConfig.lToken != address(0), "bad params");
        tokenConfig.baseUnit = baseUnit;

        emit ConfigUpdated(lToken, tokenConfig.underlying, tokenConfig.underlyingSymbol, baseUnit, tokenConfig.fixedUsd);
    }

    function updateTokenConfigFixedUsd(address lToken, bool fixedUsd) public onlyOwner {
        TokenConfig storage tokenConfig = tokenConfigs[lToken];
        require(tokenConfig.lToken != address(0), "bad params");
        tokenConfig.fixedUsd = fixedUsd;

        emit ConfigUpdated(lToken, tokenConfig.underlying, tokenConfig.underlyingSymbol, tokenConfig.baseUnit, fixedUsd);
    }
    ...
    function getOracleSourcePrice(address lToken, uint sourceIndex) public view returns (uint){

        TokenConfig storage tokenConfig = tokenConfigs[lToken];
        PriceOracle[] storage priceOracles = oracles[lToken];

        return _getUnderlyingPriceInternal(lToken, tokenConfig, priceOracles[sourceIndex]);
    }
    ...
    function oracleLength(address lToken) public view returns (uint){
        PriceOracle[] storage priceOracles = oracles[lToken];
