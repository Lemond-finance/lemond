...
contract LemdPriceOracleV1 is LemdPriceOracleInterface, OwnableUpgradeSafe {
    ...
    function getPrice(address token) external override(LemdPriceOracleInterface) view returns (uint){
        Datum storage datum = data[token];
        return datum.value;
    }

    function setPrice(address token, uint price, bool force) external override(LemdPriceOracleInterface) onlyOwner {
