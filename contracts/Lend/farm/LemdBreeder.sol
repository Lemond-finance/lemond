...
// LemdBreeder is the master of LemdToken.
contract LemdBreeder is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. LemdTokens to distribute per block.
        uint256 lastRewardBlock;  // Last block number that LemdTokens distribution occurs.
        uint256 accLemdPerShare; // Accumulated LemdTokens per share, times 1e12. See below.
    ...
    // LEMD tokens created per block.
    uint256 public lemdPerBlock;

    // The block number when LEMD mining starts.
    uint256 public startBlock;

    // The block number when LEMD claim starts.
    ...
    // Update the given pool's LEMD allocation point. Can only be called by the owner.
    ...
    // Return lemdPerBlock, baseOn power  --> lemdPerBlock * (reduceRate/100)^power
    function getLemdPerBlock(uint256 _power) public view returns (uint256){
        if (_power == 0) {
            return lemdPerBlock;
        } else {
            uint256 z = lemdPerBlock;
    ...
    // View function to see all pending LemdToken on frontend.
    function allPendingLemd(address _user) external view returns (uint256){
        uint sum = 0;
    ...
    // View function to see pending LemdToken on frontend.
    function pendingLemd(uint256 _pid, address _user) external view returns (uint256) {
    ...
    function _pending(uint256 _pid, address _user) internal view returns (uint256) {
        ...
        uint256 accLemdPerShare = pool.accLemdPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));

        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            // pending lemd reward
            uint256 lemdReward = 0;
            ...
            // get lemdReward from pool.lastRewardBlock to block.number.
            // different interval different multiplier and lemdPerBlock, sum lemdReward
            if (lastRewardBlockPower == blockNumberPower) {
                uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
                lemdReward = lemdReward.add(multiplier.mul(getLemdPerBlock(blockNumberPower)).mul(pool.allocPoint).div(totalAllocPoint));
            } else {
                for (uint256 i = lastRewardBlockPower; i <= blockNumberPower; i++) {
                    ...
                    lemdReward = lemdReward.add(multiplier.mul(getLemdPerBlock(i)).mul(pool.allocPoint).div(totalAllocPoint));
                }
            }

            accLemdPerShare = accLemdPerShare.add(lemdReward.mul(1e12).div(lpSupply));
        }

        // get pending value
        uint256 pendingValue = user.amount.mul(accLemdPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        // get lemdReward. lemdReward base on current LemdPerBlock.
        uint256 power = block.number.sub(startBlock).div(reduceIntervalBlock);
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 lemdReward = multiplier.mul(getLemdPerBlock(power)).mul(pool.allocPoint).div(totalAllocPoint);

        // mint
        lemd.mint(devAddr, lemdReward.mul(devMiningRate).div(100));
        lemd.mint(address(this), lemdReward);

        //update pool
        pool.accLemdPerShare = pool.accLemdPerShare.add(lemdReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    ...
    // Stake LP tokens to LemdBreeder for LEMD allocation.
    function stake(uint256 _pid, uint256 _amount) public {
        ...
        // if user's amount bigger than zero, transfer LemdToken to user.
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accLemdPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
                // if enableClaimBlock after block.number, save the pending to user.pendingReward.
                if (enableClaimBlock <= block.number) {
                    safeLemdTransfer(msg.sender, pending);

                    // transfer user.pendingReward if user.pendingReward > 0, and update user.pendingReward to 0
                    if (user.pendingReward > 0) {
                        safeLemdTransfer(msg.sender, user.pendingReward);
        ...
        user.rewardDebt = user.amount.mul(pool.accLemdPerShare).div(1e12);

        emit Stake(msg.sender, _pid, _amount);

    }

    // UnStake LP tokens from LemdBreeder.
