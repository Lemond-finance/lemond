...
async function main() {
    ...
    // LemdBreeder
    this.lemdBreeder = await LemdBreeder.new(this.LEMD.address, this.deployer, "1000000000000000000", "0", "100", "5760", "999", "39")
    ...
    await this.lOKB.initialize(OKB, this.comptroller.address, this.jumpRateModel.address, hre.ethers.utils.parseEther("200000000"), "Lemond OKB", "lOKB", "10")

    this.lUSDT = await LERC20.new()
    await this.lUSDT.initialize(USDT, this.comptroller.address, this.jumpRateModel.address, hre.ethers.utils.parseEther("200000000"), "Lemond USDT", "lUSDT", "10")

    this.lETHK = await LERC20.new()
    await this.lETHK.initialize(ETHK, this.comptroller.address, this.jumpRateModel.address, hre.ethers.utils.parseEther("200000000"), "Lemond ETHK", "lETHK", "10")

    this.lBTCK = await LERC20.new()
    await this.lBTCK.initialize(BTCK, this.comptroller.address, this.jumpRateModel.address, hre.ethers.utils.parseEther("200000000"), "Lemond BTCK", "lBTCK", "10")
    console.log("lTokens", this.lEther.address, this.lOKB.address, this.lETHK.address, this.lUSDT.address, this.lBTCK.address)

    // set Price
    const eth_address = "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE"
    await this.priceOracle.setPrice(eth_address, hre.ethers.utils.parseEther("210"))
    ...
    await this.lemdDistribution.initialize(this.LEMD.address, this.lemdBreeder.address, this.comptroller.address)
